...
app.directive('treeview', function() {
  return {
    ...
    link: function(scope, elem, attrs) {
		...
		const tooltipTemplate = document.querySelector('.tooltip');

		render(scope.model) // first render;

		/**
		 * [Search input]
		 */
		searchInput.addEventListener('input', e => {
			scope.searchInputValue = e.target.value;
        	doSearch(scope.searchInputValue);
		})
		...
		function addNode(item, search) {
			if(item.descricao == search){
				if(!item.nodes) {
					item.nodes = [{
						cod: scope.newNode.cod,
						descricao: scope.newNode.desc,
						obs: scope.newNode.obs
					}]
				}else{
					item.nodes.push({
						cod: scope.newNode.cod,
		...
		function doSearch(searchString){
			let dataRef = JSON.parse(JSON.stringify(scope.model)); // deep clone
            scope.searchResult = dataRef.filter(item => filterTree(item, searchString));
			render(scope.searchResult);
		}
		...
		/**
		 * [bind da função que verifica se o no da tree contem nodes]
		 */
		function isTreeNode(){
			 ...
			 Array.from(deleteNodeButton).map((item) => {
			 			item.addEventListener('click', (e) => {
								if(confirm("Deseja realmente excluir?")){
									let ref = e.target.closest('span').textContent;
									scope.model = scope.model.filter(item => deleteNode(item, ref));
									render(scope.model);
									isTreeNode();
		...
		function render(model){
			scope.template = `${model.map(treeview).join("")}`;
			wrapper.innerHTML = scope.template;
		...
		function openModal(ref){
			...
			scope.modalData.nodeName = ref; // guarda referencia do node que foi clicado
		}
		...
		function addNewNode(){
			...
			for (let [key, value] of formData.entries()) {
                scope.newNode[key] = value;
			}
			scope.model = scope.model.map(item => addNode(item, scope.modalData.nodeName));
			render(scope.model);
			closeModal();
		}
